...
- **Session titles (three sources, ordered):** 1) native `session_info_update` — applied immediately, wins over everything; 2) `session/list` polling — on the first real user message of a session that still has a placeholder title (`New Chat`/`Chat`/`Untitled`/opencode's `New session - <ISO>`), Exo polls `session/list` every 5s (max 1min) and applies the first non-placeholder title the agent reports; 3) client-side fallback — the first user message's first line (clamped ~48 chars + `…`) is shown as the tab title until a real one arrives. Polling stops on real title, timeout, disconnect, tab close/delete, or `session_info_update`. All three funnel through `applySessionTitle` (runtime + tab list + registry + UI).
...
- `chat/` — UI logic management:
    ...
    - `SessionRuntime.ts` — per-tab runtime state: its own `AcpClient`, `cwd`, `Files` cache, `messages`/`plan`/`toolCalls`/`pendingPermissions`/streaming/throttle state + title-poll state (`titlePollStart`/`titlePollTimer`). Exposes `status` (`idle`/`running`/`awaiting`) and a `SessionRuntimeCallbacks` interface (`sendPlan`/`sendMessages`/`sendTabs`/`sendStreamChunk`/`isActive`/…) supplied by ChatViewProvider.
