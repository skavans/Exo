...
- `workspaceMode.ts` — managed `.code-workspace` mode for reload-free Explorer-follow. `managedWorkspacePath`/`isManagedWorkspace` (our `<root>/.exo/exo.code-workspace`; user-owned workspaces are never touched), `pinRoot`/`getPinnedRoot` (machine-scoped root, survives the one-time migration reload), `isWorkspaceModePromptDismissed`/`dismissWorkspaceModePrompt`, `enterManagedWorkspace` (writes the workspace file + `vscode.openFolder` with `forceReuseWindow` — the single unavoidable reload), `WorkspaceFolderSwitcher` (serialized, coalescing `updateWorkspaceFolders` driver wired to `onDidChangeWorkspaceFolders`; no-ops unless in managed workspace and the target is under the pinned root).
...
- **Managed Workspace Mode (Explorer-follow):** VS Code can swap the workspace folder set WITHOUT a window reload only in a multi-root (`.code-workspace`) window; from a single-folder window any root change forces a reload. Exo manages its own workspace file at `<root>/.exo/exo.code-workspace` and, on session show/empty, live-switches its only folder to the active session's worktree (or back to the root) via `WorkspaceFolderSwitcher` → `updateWorkspaceFolders` (no reload, serialized + coalesced). Onboarding: when a single-folder git window first activates, Exo offers (once, dismissible) "Переоткрыть как воркспейс Exo" — `enterManagedWorkspace` writes the file and reloads once; the repo root is pinned in `globalState` (`exo.workspaceRoot`) BEFORE the reload and drives `getWorkspaceRoot()` — the pinned root never follows the Explorer. Guarded on `exo.followSessionFolder` (default on) and on `isManagedWorkspace` — user-owned multi-root workspaces are never touched; legacy sibling worktrees (old layout) are not followed (path must be under the pinned root).
