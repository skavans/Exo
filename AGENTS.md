...
- **Session header:** single title bar, no tab strip. Left trigger = chevron + open-session count; current session in the middle (status dot idle/running/awaiting + title + tiny agent caption); context pill (`200k/1m`, tinted static green/yellow/red by fill: <50% / ≤70% / >70%) on the right. While any session awaits permission the trigger turns warning-tinted with a pulsing awaiting-count badge. The trigger opens one merged dropdown: open sessions (status dots, close `×` on hover), "New session" entry, recent sessions (relative time, delete `×`; already-open entries filtered out). Closes on ESC / outside click; the `exo.newSession` host command opens the same menu via `showSessionPicker`. Agent identity = `title || name` + optional `v{version}`.
...
- `components/`:
    - `SessionHeader.tsx` — the title bar: trigger (chevron + session count, warning-tinted + pulsing awaiting badge), current-session block (status dot, title, agent caption), and the merged sessions dropdown (open sessions with close `×`, "New session" entry, recent list with delete `×`; ESC / outside-click close).
...
- **Session terminals:** each session gets one `exo: <title>` terminal (transient, hidden until shown, rooted in the session cwd). Color comes from a per-session index (`sessionColorIndex`, djb2 over the agent-owned `sessionId`) mapped through `SESSION_COLOR_KEYS` — the host-side mirror of the webview's `MODE_COLORS` palette (both must stay in sync). The same index is sent in `updateTabs` as `colorIndex` so the header status dots match the terminal. The label is re-synced in `applySessionTitle` (all 3 title sources call `renameSessionTerminal`), so terminals never stay stuck on `New Chat` — the rename is best-effort and guarded: `Terminal.name` is `readonly` in the VS Code API, so the assignment runs inside try/catch and only works where the runtime allows it (a throw here would otherwise abort `handleUserMessage` mid-first-message). Disposed on tab close, session delete, full disconnect and agent `onDisconnect` (crash).
