...
- **Optimistic UI / loading states:** session ops split into "optimistic" (UI state is ours) and "loader" (must wait for the agent). `closeTab`/`deleteSession` are optimistic — the tab/recent entry disappears instantly, agent teardown runs in the background. `newSession`/lazy `switchSession` cannot be optimistic (no session id / messages yet), so they show an instant loading tab (`TabStatus 'loading'`, temp id `pending-<n>`) + full-area `ChatLoading` view ("Creating/Loading session…") via `showChatLoading`, swapped for `showChat` when the runtime exists. The loading tab is not persisted, not selectable, and its `×` cancels the in-flight op (spawned agent gets disconnected, no leak). If the user navigates away while a pending op loads, resolve registers the session silently without stealing focus.
...
- **Pending Session Model (optimistic create/load):** `newSession`/lazy `switchSession` register a lightweight `PendingSession` (`pending-<n>` temp id, `mode`, `prevActiveId`, `cancelled`) in `ChatViewProvider._pendingSessions` synchronously — the webview immediately shows a loading tab + `ChatLoading` view. On success `resolvePendingSession` swaps it for the real runtime via `finishSessionOpen(runtime, title, activate)` (`activate` only when the user hasn't navigated away — otherwise the session registers silently). On failure `failPendingSession` restores `prevActiveId` (a live runtime or another pending) or `showEmpty`. Pending tabs are never persisted, never in `_tabList`/`_sessionRegistry`, not selectable in the menu, and their `×` sets `cancelled` so a late resolve disconnects the spawned agent instead of leaking it. `closeTab`/`deleteSession` are pure optimistic: the UI update happens before any agent round-trip.
...
- **Webview ↔ Host postMessage contracts (internal, non-ACP):**
    ...
    - **Host → UI:** `updateMessages`; `updateConfig`; `updatePlan`; `updateCommands`; `updateTabs` (`tabs: [{sessionId,title,status,colorIndex}]`, `activeSessionId`); `updateSessions` (recent list: `[{sessionId,title,updatedAt,active}]`); `showChatLoading` (`sessionId`,`title`,`mode: 'new'|'load'` — pending create/load, cleared by `showChat`/`showEmpty`); `showChat` (`sessionId`,`messages`,`plan`); `showEmpty`; `showSessionPicker`; `showConfigRequired`; `updateTokenUsage` (`usage`,`tokenLimit?`); `updateAgentRunning` (`running`); `searchFilesResult`; `streamChunk` (`sessionId`,`index`,`blocks`); `updateAutoAllowPermissions` (`value`); `restoreDraft` (`text`,`attachedFiles`); `updateAgentInfo` (`agentInfo`); `updatePromptCapabilities` (`{ image }`); `updateColorTheme` (`{ name }`); `resolveFileLinksResult` (`requestId`,`resolved`); `addDroppedFilesResult` (`files`,`rejected`).
