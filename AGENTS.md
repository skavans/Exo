...
- `worktree.ts` — git worktree CLI helper. `isGitRepository`, `createWorktree` (local-only `exo/<slug>` branch in a sibling `.exo-worktrees/<slug>` dir), `hasUncommittedChanges`, `sessionHasUncommittedWork` (dirty check + commits absent from main via `origin/HEAD` → local `main`/`master` fallback — used by the delete warning), `removeWorktree` (`--force`), `registerWorktreeInScm` (opens the worktree in VS Code's Source Control via the built-in git extension `api.openRepository` — worktrees live outside the workspace, so they're not auto-detected). Pure `child_process` + one vscode API call — no dependency on the VS Code git extension otherwise.
