...
- `worktree.ts` — git worktree CLI helper. `isGitRepository`, `ensureGitExclude` (adds `/.exo/` to the repo's `info/exclude` so the managed workspace + worktree copies stay invisible to git), `createWorktree` (local-only `exo/<slug>` branch in `<root>/.exo/worktrees/<slug>` — INSIDE the repo, so workspace trust keeps them trusted when the Explorer follows a session), `hasUncommittedChanges`, `sessionHasUncommittedWork` (dirty check + commits absent from main via `origin/HEAD` → local `main`/`master` fallback — used by the delete warning), `removeWorktree` (`--force`), `registerWorktreeInScm` (opens the worktree in VS Code's Source Control via the built-in git extension `api.openRepository`). Pure `child_process` + node fs + one vscode API call — no dependency on the VS Code git extension otherwise.
...
- **Worktree-per-session:** new sessions spawn their agent with a fresh `git worktree` cwd (local branch `exo/<slug>`, never pushed), living at `<root>/.exo/worktrees/<slug>` — INSIDE the repo (hidden from git via `info/exclude`). Being children of the trusted repo root keeps them workspace-trusted when the managed workspace switches the Explorer folder. Non-git workspaces fall back to the shared workspace root. `git.detectWorktrees` (on by default in VS Code) picks worktrees up in the SCM view automatically; `registerWorktreeInScm` makes them appear explicitly.
