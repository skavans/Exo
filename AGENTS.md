...
- **Session navigation:** switching / opening an item from the header menu is a pure UI view change — live ACP sessions keep running in the background. Closing a session (`×` in the menu) kills the agent process; the session remains in the recent list. Only `deleteSession` removes it permanently (and asks about the worktree if it has uncommitted changes).
