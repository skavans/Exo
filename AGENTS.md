...
- **Session header:** single row (back button, title with agent label inline, token count). Thin absolute-positioned context progress bar at the bottom edge. Session-list heading uses the same inline title + agent-label layout. Agent identity = `title || name` + optional `v{version}`.
- **Session navigation:** switching chat → session list is a pure UI view change; it does NOT close the live ACP session. Reopening the chat returns to the same live session.
...
- Extension Host persists two separate `workspaceState` entries via debounced saves: `exo.chatUiState` (view mode, active session id/title pointer) and `exo.chatDraft` (textarea text + attached file paths — no images).
- On webview startup (`handleReady`), host restores these, reconnects to agent, and reopens the persisted session via `session/load` or `session/resume`. Session content comes from agent replay and remains agent-owned.
...
### Extension Host (`src/`)
- `extension.ts` — entry point: ensures config file, starts `ConfigWatcher`, creates and registers `ChatViewProvider`, registers commands (`openChat`, `newSession`, `openConfig`), subscribes to `onDidChangeActiveColorTheme`.
...
- `acp/` — core of agent communication:
    ...
    - `handlers/util.ts` — shared utilities: `extractToolText`, `extractDiffContent` (standard ACP `type:'diff'`), `applyToolCallPatch` (upsert `ToolCallInfo` from `ToolCallUpdate`), `extractPlanFromToolArgs` (vendor fallback: detects plan by `{todos:[...]}` shape), `extractEditSpec`, `EditSpec`/`ToolCallRegistryContext` types.
- `chat/` — UI logic management:
    - `ChatViewProvider.ts` — bridge between ACP client and Webview. Holds session runtime state (`messages`, `currentPlan`, `currentUsage`, `availableCommands`, `mode`, `pendingPermissions`, `toolCallInfos`, streaming/throttle state). Tracks current UI view (`list`/`chat`). Persists active-session pointer and draft in `workspaceState`. Implements permission auto-allow, reject-with-response optimistic queuing, plan mapping (`PlanEntry[]` → UI `Plan`, `completed`→`done`), config-selector wiring, file-link resolution, dropped-file validation. Registers `exo-diff:` content provider.
...
- `files/Files.ts` — flat line cache (`read(from,end)`, `replaceFull`). Uses `splitLines`/`resolveAndValidatePath` from `tools/utils.ts`. Legacy revealed-ranges/LRU-eviction removed.
- `tools/` — helpers:
...
- `App.tsx` — root component. Holds top-level state (`messages`, `config`, `plan`, `commands`, `sessionList`, `agentInfo`, `sessionId`, `tokenUsage`/`tokenLimit`, `isAgentRunning`, `autoAllowPermissions`, `promptCapabilities`, `colorThemeName`, `themeVersion`). Routes `postMessage` events. `MessageList` mounted with `key={sessionId}` (clean remount on session switch, resetting scroll/ErrorBoundary). Computes `pendingReject` (last awaiting-permission reject option) and passes to `MessageInput`.
...
- `types.ts` — UI-side data contracts: `ChatMessage`, `MessageBlock`, `ToolCallInfo`, `AttachedImage`, `Plan`/`PlanItem`, `AcpSessionInfo`, `CommandInfo`, `AgentInfo`, `ConfigState`/`ConfigSelector`/`ConfigOption`, `MODE_COLORS` (ANSI-terminal token references), helpers (`formatAgentLabel`, `formatCompactNumber`, `modeColor`, `useActiveModeColor`).
- `components/`:
...
- **Two-level Session Persistence:** Short-lived continuity (sidebar view switches) via `retainContextWhenHidden`; longer-lived continuity (VS Code restarts) via `workspaceState` storing only client-owned UI state (active session pointer, view, draft). Message history and plan remain agent-owned, restored via ACP replay. Two separate debounced keys: `exo.chatUiState`, `exo.chatDraft`.
- **Edit Review Flow:** For edit-permission requests, client opens VS Code Diff Editor (`exo-diff:` scheme). Diff content (original/proposed) comes exclusively from the standard ACP `type:'diff'` content block within `requestPermission.toolCall.content`. No vendor-specific unified-diff or args-based fallback.
...
- **Session List vs Session Close:** `showSessionList` = navigation only. Closing the live ACP session is a separate host action (`closeCurrentSession`), decoupled from returning to list UI.
